...
=end
      ...
      private
        ...
        def check_tree
          if !@trees
            ...
            data['tree'].each { |k, d| @trees[k] = GitRuby::Object::Tree.new(@base, d[:sha], d[:mode]) }
            data['blob'].each { |k, d| @blobs[k] = GitRuby::Object::Blob.new(@base, d[:sha], d[:mode]) }
          end
        end
    ...
    class Commit < AbstractObject
      ...
      def set_commit(data)
        ...
        @committer = GitRuby::Author.new(data['committer'])
        @author = GitRuby::Author.new(data['author'])
        @tree = GitRuby::Object::Tree.new(@base, data['tree'])
        @parents = data['parent'].map{ |sha| GitRuby::Object::Commit.new(@base, sha) }
    ...
    class << self
      ...
      def new(base, objectish, type = nil, is_tag = false)
        if is_tag
          ...
          if sha == ''
            raise GitRuby::GitTagNameDoesNotExist.new(objectish)
          end
          return GitRuby::Object::Tag.new(base, sha, objectish)
        else
          if !type
